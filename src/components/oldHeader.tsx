...
interface HeaderProps {
  // Optional prop to pass authenticated state, defaults to true for preview
  initialLoggedIn?: boolean;
}

export default function Header({ initialLoggedIn = true }: HeaderProps) {
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-slate-200/80 bg-white/80 backdrop-blur-md transition-colors duration-200 dark:border-slate-800/80 dark:bg-slate-950/80">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <Link 
          href="/" 
          className="group flex items-center gap-2.5 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 rounded-lg dark:focus:ring-offset-slate-900"
        >
        ...
        <div className="flex items-center gap-3 sm:gap-4">
          ...
          {/* Demo State Switcher (Optional - For testing logged in/out) */}
          {/* <button
            onClick={() => setIsLoggedIn(!isLoggedIn)}
            className="hidden text-xs text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 sm:block mr-2"
            title="Toggle state preview"
          >
          ...
          {isLoggedIn ? (
            ...
            <div className="relative" ref={dropdownRef}>
              ...
              >
                <div className="relative flex h-9 w-9 items-center justify-center rounded-full bg-slate-100 font-semibold text-slate-700 ring-1 ring-slate-200 dark:bg-slate-800 dark:text-slate-200 dark:ring-slate-700">
              ...
              {isDropdownOpen && (
                <div className="absolute right-0 mt-2 w-56 origin-top-right rounded-xl border border-slate-200/80 bg-white p-1.5 shadow-xl transition-all duration-200 dark:border-slate-800/80 dark:bg-slate-900">
                  ...
                  <div className="border-b border-slate-100 px-3 py-2.5 dark:border-slate-800/60">
                    ...
                    <p className="truncate text-sm font-semibold text-slate-900 dark:text-slate-100">john.doe@focusflow.io</p>
                  </div>
